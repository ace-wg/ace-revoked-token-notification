...
Following a request to the TRL endpoint, the response messages sent by the AS use Content-Format "application/ace-trl+cbor", unless otherwise specified. Their payload is formatted as a CBOR map, and the CBOR values for the parameters included therein are defined in {{trl-registry-parameters}}.
